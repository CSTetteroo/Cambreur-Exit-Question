<x-app-layout>
    ...
    <div class="py-8 bg-gray-900 min-h-screen text-gray-100">
        <div class="max-w-6xl mx-auto px-4 space-y-8">
            @if (session('status'))
                <div class="p-3 rounded bg-green-800/60 border border-green-700 text-green-100">{{ session('status') }}
                </div>
            @endif
                                    ...
                                    <tr class="bg-gray-800/40">
                                        <td class="px-4 py-3 align-top">
                                            <button @click.stop="open = !open" type="button"
                                                class="inline-flex items-center px-2 py-1 text-xs rounded bg-gray-700 hover:bg-gray-600">
                                                <svg x-show="!open" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                        ...
                                        <td class="px-4 py-3 align-top">
                                            <div class="font-medium max-w-[60ch] truncate" title="{{ e($q->content) }}">
                                                {{ Str::limit(str_replace(["\r\n", "\n"], ' ', $q->content), 35) }}</div>
                                        </td>
                                        <td class="px-4 py-3 align-top">
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded bg-indigo-600/20 text-indigo-300 border border-indigo-600/30 text-xs">{{ $q->type === 'multiple_choice' ? 'Meerkeuze' : 'Open' }}</span>
                                        </td>
                                        <td class="px-4 py-3 align-top text-sm text-gray-300">{{ $q->choices_count }}
                                        </td>
                                        ...
                                        <td colspan="6" class="px-6 pb-6 pt-2">
                                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                <div>
                                                    ...
                                                    <div
                                                        class="p-3 rounded border border-gray-700 bg-gray-800/60 break-words overflow-auto max-h-60">
                                                        {!! nl2br(e($q->content)) !!}</div>
                                                    @if ($q->type === 'multiple_choice' && $q->choices->isNotEmpty())
                                                        <div class="mt-4">
                                                            <div class="text-sm text-gray-400 mb-2">Opties</div>
                                                            <ul
                                                                class="text-sm text-gray-300 list-disc pl-5 break-words">
                                                                @foreach ($q->choices as $ch)
                                                                    <li>
                                                                        <span
                                                                            class="text-gray-400">{{ $ch->label }}.</span>
                                                                        {{ $ch->text }}
                                                                        @if ($ch->is_correct)
                                                                            <span
                                                                                class="ml-2 text-emerald-400 text-xs">(juist)</span>
                                                                        @endif
                                                                    </li>
                                                                @endforeach
                                                            </ul>
                                                        ...
                                                        <form method="POST"
                                                            action="{{ route('docent.questions.setCorrect', $q) }}"
                                                            class="mt-3">
                                                            @csrf
                                                            ...
                                                            <button
                                                                class="mt-3 px-4 py-1.5 rounded bg-emerald-600 hover:bg-emerald-700 text-sm">Opslaan</button>
                                                        </form>
                                                    @endif
                                                </div>
                                                <div>
                        ...
                        <div>
                            ...
                            <div class="mt-3 flex items-center gap-3">
                                <button type="button" @click="addChoice()"
                                    class="px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600 text-sm disabled:opacity-50"
                                    :disabled="choices.length >= 4">Optie toevoegen</button>
